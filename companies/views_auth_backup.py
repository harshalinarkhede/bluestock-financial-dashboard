...
@api_view(['GET'])
def drf_companies_list(request):
...
@api_view(['GET'])
def drf_profitandloss_list(request):
    profit_data = Profitandloss.objects.all()[:20]
    serializer = ProfitandlossSerializer(profit_data, many=True)
    return Response(serializer.data)